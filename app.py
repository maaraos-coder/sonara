...
APP_VERSION = "1.0.2 TRIAL LIMITS"
...
def app_cubicacion():
    ...
    st.markdown(
        "<div class='result-note'>Cubicación preliminar de soluciones acústicas. Permite descontar vanos de puertas/ventanas y asociar la cubicación a una solución o proyecto.</div>",
        unsafe_allow_html=True
    )
    ...
    col_in, col_out = st.columns([1.0, 1.25], gap="large")

    with col_in:
        st.markdown("<div class='sonara-card-title'>Identificación</div>", unsafe_allow_html=True)
        nombre_solucion = st.text_input("Nombre de la solución / partida", value="Tabique acústico")
        codigo_partida = st.text_input("Código o referencia", value="Proyecto-01")

        st.markdown("<hr>", unsafe_allow_html=True)
        st.markdown("<div class='sonara-card-title'>Geometría</div>", unsafe_allow_html=True)
        largo = st.number_input("Largo del tabique [m]", value=5.0, min_value=0.5, step=0.5)
        alto = st.number_input("Alto del tabique [m]", value=2.4, min_value=1.0, step=0.1)
        area_bruta = largo * alto

        incluir_vanos = st.checkbox("Descontar puertas/ventanas", value=False)
        area_vanos = 0.0

        if incluir_vanos:
            n_puertas = st.number_input("N° puertas", value=0, min_value=0, max_value=20, step=1)
            ancho_puerta = st.number_input("Ancho puerta tipo [m]", value=0.8, min_value=0.1, step=0.1)
            alto_puerta = st.number_input("Alto puerta tipo [m]", value=2.0, min_value=0.5, step=0.1)

            n_ventanas = st.number_input("N° ventanas", value=0, min_value=0, max_value=50, step=1)
            ancho_ventana = st.number_input("Ancho ventana tipo [m]", value=1.2, min_value=0.1, step=0.1)
            alto_ventana = st.number_input("Alto ventana tipo [m]", value=1.0, min_value=0.1, step=0.1)

            area_vanos = n_puertas * ancho_puerta * alto_puerta + n_ventanas * ancho_ventana * alto_ventana

        area = max(area_bruta - area_vanos, 0.1)

        st.markdown("<hr>", unsafe_allow_html=True)
        config = st.selectbox("Configuración", ["Montante simple", "Montantes independientes", "Doble estructura"], index=0)
        camara = st.number_input("Cámara / estructura [mm]", value=70.0, min_value=40.0, step=10.0)
        absorbente = st.selectbox("Absorbente", ["Sin absorbente", "Lana mineral 40 kg/m³", "Lana mineral 60 kg/m³", "Lana mineral 80 kg/m³"], index=2)
        perdida = st.slider("Pérdida de material [%]", min_value=0, max_value=25, value=10) / 100.0

        layers_left = layer_inputs("cub_left", "Lado 1", materiales_panel, mats, default_count=1, max_layers=4)
        layers_right = layer_inputs("cub_right", "Lado 2", materiales_panel, mats, default_count=1, max_layers=4)

    df, total = estimate_cubicacion_tabique(
        layers_left, layers_right, area, largo, alto, camara, config, absorbente, waste=perdida
    )
    ...
    with col_out:
        st.markdown(
            ...
            <div class='sonara-card' style='margin-bottom:16px;'>
                ...
                <p style='color:#DCEBFF;margin:0;'>Referencia: <b>{codigo_partida}</b> · Área bruta: <b>{area_bruta:.1f} m²</b> · Vanos descontados: <b>{area_vanos:.1f} m²</b> · Área neta: <b>{area:.1f} m²</b></p>
            </div>
            <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:20px;">
                ...
                <div class="result-card"><div class="result-label">Masa</div><div class="result-value">{masa:.1f}</div><div class="result-unit">kg/m²</div></div>
                <div class="result-card"><div class="result-label">Espesor</div><div class="result-value">{espesor:.0f}</div><div class="result-unit">mm</div></div>
        ...
        st.download_button(
            ...
            file_name="sonara_cubicacion.csv",
            mime="text/csv",
            use_container_width=True
        )
...
def app_proyectos():
    ...
    with tab_costos:
        ...
        if sol and req:
            st.markdown(f"<div class='result-note'>Cubicando: <b>{sol.get('nombre')}</b> para <b>{req.get('recinto_emisor')} → {req.get('recinto_receptor')}</b>.</div>", unsafe_allow_html=True)
        else:
            st.markdown("<div class='result-note'>Selecciona una solución guardada para cubicarla o usa cubicación general.</div>", unsafe_allow_html=True)
        app_cubicacion()
...
def app_ayuda():
    ...
    tabs = st.tabs(["🚀 Inicio rápido", "📘 Conceptos", "🏗️ Casos de uso", "📚 Referencias", "🛟 Soporte"])
