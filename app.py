...
APP_VERSION = "1.2.0 ESTABLE + ESQUEMA VISUAL"
...
def sonara_layer_material_name(layer):
    ...
    Evita 'Material no identificado' cuando la capa trae el material sin campo 'nombre'.
    """
    if not isinstance(layer, dict):
    ...
    if isinstance(mat, dict):
        for k in ["nombre", "name", "material", "tipo", "label"]:
            ...
            if val not in [None, ""]:
                txt = str(val)
                if txt.lower() not in ["material", "none", "nan"]:
                    return txt

        # Inferir por comparación con biblioteca de materiales
        try:
            mats = material_db()
            m_dens = sonara_safe_float(mat.get("dens", mat.get("densidad", 0)), 0)
            m_esp = sonara_safe_float(mat.get("espesor", 0), 0)
            m_precio = sonara_safe_float(mat.get("precio", 0), 0)
            best_name = None
            best_score = 999999
            for name, ref in mats.items():
                score = 0
                score += abs(sonara_safe_float(ref.get("dens", 0), 0) - m_dens) / 50 if m_dens else 100
                score += abs(sonara_safe_float(ref.get("espesor", 0), 0) - m_esp) / 5 if m_esp else 10
                if m_precio:
                    score += abs(sonara_safe_float(ref.get("precio", 0), 0) - m_precio) / 1000
                if score < best_score:
                    best_score = score
                    best_name = name
            if best_name and best_score < 25:
    ...
    for k in ["nombre", "name", "material", "material_nombre", "tipo", "label"]:
        val = layer.get(k)
        if val not in [None, ""]:
            txt = str(val)
            if txt.lower() not in ["material", "none", "nan"]:
...
def sonara_draw_solution_html(solution):
    """
    Esquema visual HTML de solución optimizada o diseñada.
    Se muestra en Optimización IA y sirve para entender qué capas componen la solución.
    """
    if not isinstance(solution, dict):
        return ""

    layers_left = solution.get("layers_left", []) or solution.get("capas_izq", []) or solution.get("layers", []) or solution.get("capas", [])
    ...
    def layer_block(layer):
        nombre = sonara_html_escape(sonara_layer_material_name(layer))
        ...
        width = max(24, min(90, esp * 2.2 if esp > 0 else 30))
        return f"""
        <div title="{nombre}" style="
            width:{width}px;
            height:190px;
            background:linear-gradient(180deg,{color},#6B7280);
            ...
            margin:0 4px;
            box-shadow: inset 0 0 14px rgba(255,255,255,.10), 0 10px 22px rgba(0,0,0,.25);">
            {nombre}<br>{esp:g} mm
        </div>
        """
    ...
    for layer in layers_left:
        html_layers += layer_block(layer)

    if camara > 0:
        abs_txt = sonara_html_escape(absorbente or "Cámara de aire")
        html_layers += f"""
        <div style="
            width:{max(42, min(110, camara))}px;
            height:190px;
            background:repeating-linear-gradient(
                45deg,
                rgba(250,204,21,.76),
                rgba(250,204,21,.76) 6px,
                rgba(15,23,42,.35) 6px,
                rgba(15,23,42,.35) 14px
            );
    ...
    <div class="sonara-card" style="margin-top:18px;">
        ...
        <div style="
            ...
            min-height:245px;
            background:#07111d;
            border:1px solid rgba(71,166,255,.25);
            border-radius:14px;
            padding:20px;
            overflow-x:auto;">
            {html_layers}
...
def sonara_cubicacion_from_optimized_solution(opt_solution, area_m2=1.0):
    ...
    Lee correctamente los nombres/precios de los materiales aunque las capas vengan en formatos antiguos.
    """
    ...
    try:
        ...
        for k, v in mats.items():
            if "mano de obra" in str(k).lower():
                precio_mo = sonara_safe_float(v.get("precio", 0), 0)
                items.append({
...
def app_optimizador():
    ...
    if results:
        ...
        st.markdown(sonara_draw_solution_html(selected_variant), unsafe_allow_html=True)
        ...
        if st.button("💾 Guardar variante optimizada en el requerimiento", use_container_width=True):
            ...
            opt_solution = {
                ...
                "esquema_html": sonara_draw_solution_html(selected_variant),
            }
