...
APP_VERSION = "1.1.1 COSTOS + LOSCAA"
...
def app_loscaa_validation():
    st.markdown(
        "<h2 style='color:#FFFFFF;margin-bottom:18px;'>Validación LOSCAA — MINVU E13 2024</h2>",
        unsafe_allow_html=True
    )
    ...
    st.markdown(
        """
        <div class='result-note'>
        Comparación preliminar entre soluciones del Listado Oficial de Soluciones Constructivas para Acondicionamiento Acústico
        del MINVU y el motor SONARA. Esta herramienta sirve para calibración y control técnico del modelo.
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    with col_left:
        st.markdown("<div class='sonara-card-title'>Biblioteca de casos</div>", unsafe_allow_html=True)
        ...
        selected = st.selectbox("Seleccionar solución LOSCAA", opciones)
        ...
        with c1:
            if st.button("💾 Guardar biblioteca", use_container_width=True):
                st.session_state["loscaa_editor"].to_csv(LOSCAA_CSV, index=False, encoding="utf-8")
    ...
    with col_right:
        ...
        st.markdown("<div class='sonara-card-title'>Tabla comparativa</div>", unsafe_allow_html=True)
        ...
        st.markdown(
            """
            <div class='result-note'>
            Escala de validación sugerida: ±1 dB excelente · ±2 dB muy bueno · ±3 dB aceptable · &gt;3 dB revisar modelo.
            Las soluciones de ventana corredera incluyen penalización referencial por marco/fugas para aproximar el ensayo real.
            </div>
            """,
            unsafe_allow_html=True
        )
        ...
        st.download_button(
            ...
            file_name=f"sonara_validacion_{selected_code}.csv",
            mime="text/csv",
            use_container_width=True
        )
...
def app_ayuda():
    ...
    with tabs[4]:
        st.markdown("<div class='sonara-card-title'>Validación referencial LOSCAA 2024</div>", unsafe_allow_html=True)
        st.markdown(
            """
            <div class='project-flow-card'>
                <div class='project-flow-title'>Alcance de validación SONARA</div>
                <div class='project-flow-text'>
                SONARA incorpora una validación referencial basada en el <b>Listado Oficial de Soluciones Constructivas para
                Acondicionamiento Acústico del Ministerio de Vivienda y Urbanismo, ED13 2024</b>, aprobado por
                Resolución Exenta N°786 del 31 de mayo de 2024.<br><br>
                Esta contrastación verifica cobertura normativa, clasificación de familias constructivas y capacidad de
                representación de soluciones equivalentes. No reemplaza informes de ensayo de laboratorio, mediciones in situ
        ...
        try:
            if not VALIDACION_LOSCAA_CSV.exists():
                default_validacion_loscaa_dataframe().to_csv(VALIDACION_LOSCAA_CSV, index=False, encoding="utf-8")
            df_val = pd.read_csv(VALIDACION_LOSCAA_CSV)
        except Exception:
            df_val = default_validacion_loscaa_dataframe()

        familias = df_val["familia"].nunique() if "familia" in df_val.columns else 0
        casos = len(df_val)
        tipos = df_val["tipo"].nunique() if "tipo" in df_val.columns else 0

        st.markdown(
            f"""
            <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:20px;">
                <div class="result-card"><div class="result-label">Fuente</div><div class="result-value" style="font-size:22px;">LOSCAA</div><div class="result-unit">MINVU 2024</div></div>
                <div class="result-card"><div class="result-label">Casos</div><div class="result-value">{casos}</div><div class="result-unit">referenciales</div></div>
                <div class="result-card"><div class="result-label">Familias</div><div class="result-value">{familias}</div><div class="result-unit">constructivas</div></div>
                <div class="result-card"><div class="result-label">Tipos</div><div class="result-value">{tipos}</div><div class="result-unit">elementos</div></div>
            </div>
            """,
            unsafe_allow_html=True
        )

        if "familia" in df_val.columns:
            sel_fam = st.multiselect(
                "Filtrar familias",
                sorted(df_val["familia"].dropna().unique().tolist()),
                default=sorted(df_val["familia"].dropna().unique().tolist()),
                key="val_loscaa_familias"
            )
            df_show = df_val[df_val["familia"].isin(sel_fam)].copy()
        else:
            df_show = df_val.copy()
        ...
        st.download_button(
            "⬇ Descargar validación LOSCAA CSV",
            data=df_show.to_csv(index=False).encode("utf-8"),
            file_name="validacion_loscaa_sonara.csv",
            mime="text/csv",
            use_container_width=True
        )
        ...
        st.markdown(
            """
            <div class='result-note'>
            <b>Nota técnica:</b> esta sección corresponde a validación referencial y normativa. La validación experimental
            del motor requiere informes de ensayo con valores medidos por bandas o, al menos, Rw/Ln,w medido para cada solución.
            </div>
            """,
            unsafe_allow_html=True
        )
