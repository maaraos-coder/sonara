...
APP_VERSION = "1.1.5 OPTIMIZACION CUBICABLE + INFORME"
...
def sonara_build_variant_record(nombre, estrategia, layers_left, layers_right=None, camara_mm=0, absorbente_tipo="Sin absorbente", configuracion="Montante simple", area_m2=1.0):
    ...
    m1, e1 = sonara_layers_mass_thickness(layers_left or [])
    m2, e2 = sonara_layers_mass_thickness(layers_right or [])
    masa = m1 + m2
    espesor = e1 + e2 + float(camara_mm or 0)
    costo = sonara_variant_cost_from_layers(layers_left or [], area_m2) + sonara_variant_cost_from_layers(layers_right or [], area_m2)
    return {
        "nombre": nombre,
        "estrategia": estrategia,
        "layers_left": layers_left or [],
        "layers_right": layers_right or [],
        "camara_mm": float(camara_mm or 0),
        "absorbente_tipo": absorbente_tipo,
        "configuracion": configuracion,
        "masa": float(masa),
        "espesor": float(espesor),
...
def sonara_base_layers_from_context(solution=None, materiales=None):
    """
    Intenta extraer las capas desde la solución seleccionada.
    Si no encuentra capas, crea una base razonable según material disponible.
    """
    materiales = materiales or material_db()

    # Intentar desde estructura guardada
    if isinstance(solution, dict):
        for key in ["layers_left", "capas", "layers", "capas_izq"]:
            val = solution.get(key)
            if isinstance(val, list) and len(val):
                return val

        # Si la solución tiene composición textual o nombre
        name = str(solution.get("nombre", solution.get("solution_name", solution.get("tipo", "")))).lower()
    else:
        name = ""

    # Fallback según biblioteca
    mat_hormigon = sonara_get_material_by_keywords(materiales, ["hormigón", "hormigon"])
    mat_yeso = sonara_get_material_by_keywords(materiales, ["yeso-cartón estándar", "yeso"])
    mat_alta = sonara_get_material_by_keywords(materiales, ["alta densidad", "habito"], fallback=mat_yeso)
    mat_vidrio = sonara_get_material_by_keywords(materiales, ["vidrio monolítico", "vidrio"])

    if "hormig" in name and mat_hormigon:
        return [{"material": materiales[mat_hormigon], "espesor": 110.0, "densidad": materiales[mat_hormigon].get("dens", 2340)}]
    if "vidrio" in name or "ventana" in name:
        return [{"material": materiales[mat_vidrio], "espesor": 5.0, "densidad": materiales[mat_vidrio].get("dens", 2500)}]
    if mat_yeso:
        return [{"material": materiales[mat_yeso], "espesor": 15.0, "densidad": materiales[mat_yeso].get("dens", 735)}]
    if mat_alta:
        return [{"material": materiales[mat_alta], "espesor": 15.0, "densidad": materiales[mat_alta].get("dens", 786)}]
    return []
...
def sonara_generate_progressive_variants(base_solution=None, target_rw=45, max_espesor=150, max_masa=80, area_m2=1.0, modo="Rápido"):
    ...
    2) Agregar capa.
    3) Agregar cámara + absorbente.
    4) Desacople / doble estructura.
    Calcula pocas variantes plausibles y luego filtra por masa/espesor.
    """
    ...
    mat_yeso = sonara_get_material_by_keywords(materiales, ["yeso-cartón estándar", "yeso"])
    mat_hd = sonara_get_material_by_keywords(materiales, ["alta densidad", "habito"], fallback=mat_yeso)
    mat_lana40 = sonara_get_material_by_keywords(materiales, ["lana mineral 40"], fallback=None)
    mat_lana60 = sonara_get_material_by_keywords(materiales, ["lana mineral 60", "lana"], fallback=mat_lana40)

    candidates = []

    # Base
    candidates.append(sonara_build_variant_record(
        "Solución base", "Base existente", base_left, [], 0, "Sin absorbente", "Montante simple", area_m2
    ))

    # 1. Optimización mínima: agregar masa aumentando espesor o densidad
    for factor in ([1.10, 1.20, 1.35] if modo == "Rápido" else [1.05, 1.10, 1.20, 1.35, 1.50]):
        new_layers = []
        for l in base_left:
            nl = dict(l)
            nl["espesor"] = round(float(l.get("espesor", 0) or 0) * factor, 1)
            new_layers.append(nl)
        candidates.append(sonara_build_variant_record(
            f"+ masa / espesor x{factor:.2f}", "1. Agregar masa a capas existentes", new_layers, [], 0, "Sin absorbente", "Montante simple", area_m2
        ))

    # 2. Agregar una nueva capa al lado más débil
    if mat_hd:
        hd = materiales[mat_hd]
    ...
    if mat_yeso:
        ...
        for cam in ([50, 70] if modo == "Rápido" else [40, 50, 70, 90]):
            for abs_tipo in (["Lana mineral 60 kg/m³"] if modo == "Rápido" else ["Lana mineral 40 kg/m³", "Lana mineral 60 kg/m³", "Lana mineral 80 kg/m³"]):
                candidates.append(sonara_build_variant_record(
    ...
    if mat_hd:
        ...
        for conf in (["Montantes independientes", "Doble estructura"] if modo != "Rápido" else ["Montantes independientes"]):
            candidates.append(sonara_build_variant_record(
    ...
    # Filtrar preliminarmente por restricciones y ordenar por cercanía técnica/costo
    filtered = []
    for c in candidates:
        if c["espesor"] <= float(max_espesor or 9999) and c["masa"] <= float(max_masa or 9999):
            filtered.append(c)

    if not filtered:
        filtered = candidates[:]

    # Preorden: cumple preliminar objetivo, menor costo, menor espesor
    filtered = sorted(
        filtered,
        key=lambda x: (
            0 if x.get("rw_preliminar", 0) >= float(target_rw or 0) else 1,
            abs(float(target_rw or 0) - x.get("rw_preliminar", 0)),
...
def sonara_precise_score_variant(variant):
    ...
    Retorna variante con Rw, C, Ctr y estado.
    """
    freqs_fine = np.linspace(50, 5000, 900)
    bands = np.array([50, 63, 80, 100, 125, 160, 200, 250, 315, 400, 500, 630, 800, 1000, 1250, 1600, 2000, 2500, 3150, 4000, 5000])

    try:
        left = variant.get("layers_left", [])
        right = variant.get("layers_right", [])
        cam = float(variant.get("camara_mm", 0) or 0)
        ...
        variant["Rw+C"] = 0.0
        variant["Rw+Ctr"] = 0.0
        variant["resultado"] = f"Error cálculo: {e}"
        return variant
...
def sonara_run_fast_optimizer(base_solution=None, target_rw=45, max_espesor=150, max_masa=80, area_m2=1.0, prioridad="Menor costo", modo="Rápido"):
    """
    Orquestador de optimización rápida.
    Primero genera variantes técnicamente razonables y solo calcula WALLS en candidatas filtradas.
    """
...
# =========================================================
# SONARA 1.1.5 - HELPERS OPTIMIZACION CUBICABLE
# =========================================================
...
def sonara_cubicacion_from_optimized_solution(opt_solution, area_m2=1.0):
    """
    Genera una cubicación inicial editable para una solución optimizada.
    Esta cubicación queda dentro de opt_solution['cubicacion'] y luego el módulo Cubicación
    puede editarla y persistirla en el proyecto.
    """
    area = sonara_safe_float(area_m2, 1.0)
    items = []

    for lado, layers in [("Lado 1", opt_solution.get("layers_left", [])), ("Lado 2", opt_solution.get("layers_right", []))]:
        for layer in layers or []:
            mat = layer.get("material", {}) if isinstance(layer, dict) else {}
            nombre = sonara_material_nombre(mat)
            unidad = sonara_material_unidad(mat)
            precio = sonara_material_precio(mat)
            esp = sonara_safe_float(layer.get("espesor", 0), 0) if isinstance(layer, dict) else 0
        ...
        try:
            ...
            for k, v in mats.items():
                if str(abs_tipo).lower() in str(k).lower() or str(k).lower() in str(abs_tipo).lower():
                    precio_abs = sonara_material_precio(v)
                    break
        except Exception:
            precio_abs = 0.0
        items.append({
    ...
    # Mano de obra referencial si existe en la biblioteca
    try:
        mats = material_db()
        for k, v in mats.items():
            if "mano de obra" in str(k).lower():
                precio_mo = sonara_material_precio(v)
                items.append({
    ...
    total = 0.0
    for it in items:
        cantidad = sonara_safe_float(it.get("Cantidad", 0), 0)
        precio = sonara_safe_float(it.get("Precio unitario", 0), 0)
        it["Cantidad"] = cantidad
        it["Precio unitario"] = precio
        it["Subtotal"] = round(cantidad * precio, 0)
        total += sonara_safe_float(it.get("Subtotal", 0), 0)
