...
def loscaa_validation_dataframe(df=None):
    """
    Ejecuta todos los casos LOSCAA que tienen un tipo_sonara implementado y calcula errores estadísticos.
    Usa los descriptores oficiales incorporados en data/loscaa_validacion.csv.
    """
    if df is None:
    ...
    for _, row in df.iterrows():
        ...
        if metric == "Ln,w":
            sonara = result.get("lnw")
            oficial = None if pd.isna(lnw_of) else float(lnw_of)
            descriptor = "Ln,w"
            sonara_aux = result.get("lnw_ci")
            oficial_aux = None if pd.isna(lnwci_of) else float(lnwci_of)
            descriptor_aux = "Ln,w+CI"
        else:
            sonara = result.get("rw")
            oficial = None if pd.isna(rw_of) else float(rw_of)
            descriptor = "Rw"
            if result.get("rw") is not None and result.get("ctr") is not None:
                sonara_aux = float(result.get("rw")) + float(result.get("ctr"))
            else:
                sonara_aux = None
            oficial_aux = None if pd.isna(rw_of) or pd.isna(ctr_of) else float(rw_of) + float(ctr_of)
            descriptor_aux = "Rw+Ctr"

        delta = None if sonara is None or oficial is None else float(sonara) - float(oficial)
        delta_aux = None if sonara_aux is None or oficial_aux is None else float(sonara_aux) - float(oficial_aux)

        rows.append({
            "Código": row.get("codigo", ""),
            "Solución": row.get("nombre", ""),
            "Familia": row.get("familia", ""),
            "Descriptor": descriptor,
            "LOSCAA": oficial,
            "SONARA": None if sonara is None else float(sonara),
            "Error [dB]": delta,
            "Abs error [dB]": None if delta is None else abs(delta),
            "Estado": loscaa_status(delta),
            "Descriptor auxiliar": descriptor_aux,
            "LOSCAA auxiliar": oficial_aux,
            "SONARA auxiliar": sonara_aux,
            "Error auxiliar [dB]": delta_aux,
            "Nota": row.get("nota", ""),
        })
...
def loscaa_validation_stats(df_res):
    """Calcula métricas globales para validación SONARA vs LOSCAA."""
    if df_res is None or df_res.empty or "Error [dB]" not in df_res.columns:
        return {"n": 0, "mae": 0.0, "rmse": 0.0, "max_abs": 0.0, "bias": 0.0, "within3": 0.0, "within5": 0.0, "r2": None}

    work = df_res.copy()
    work["Error [dB]"] = pd.to_numeric(work["Error [dB]"], errors="coerce")
    work["LOSCAA"] = pd.to_numeric(work["LOSCAA"], errors="coerce")
    work["SONARA"] = pd.to_numeric(work["SONARA"], errors="coerce")
    work = work.dropna(subset=["Error [dB]", "LOSCAA", "SONARA"])

    if work.empty:
        return {"n": 0, "mae": 0.0, "rmse": 0.0, "max_abs": 0.0, "bias": 0.0, "within3": 0.0, "within5": 0.0, "r2": None}

    err = work["Error [dB]"].astype(float)
    abs_err = err.abs()
    n = len(work)
    ...
    bias = float(err.mean())
    within3 = float((abs_err <= 3).mean() * 100.0)
    within5 = float((abs_err <= 5).mean() * 100.0)

    r2 = None
    try:
        if n >= 2 and work["LOSCAA"].std() > 0 and work["SONARA"].std() > 0:
            r = float(np.corrcoef(work["LOSCAA"], work["SONARA"])[0, 1])
            r2 = r ** 2
    except Exception:
        r2 = None

    return {"n": n, "mae": mae, "rmse": rmse, "max_abs": max_abs, "bias": bias, "within3": within3, "within5": within5, "r2": r2}
...
def app_loscaa_validation():
    ...
    st.markdown(
        """
        <div class='result-note'>
        Comparación cuantitativa entre descriptores oficiales LOSCAA y el motor SONARA. La validación se calcula solo con casos cuyo método está implementado en SONARA.
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    df = ensure_loscaa_file()
    df_res = loscaa_validation_dataframe(df)
    stats = loscaa_validation_stats(df_res)

    r2_txt = "-" if stats["r2"] is None else f"{stats['r2']:.2f}"
    st.markdown(
        f"""
        <div style="display:grid;grid-template-columns:repeat(6,1fr);gap:12px;margin-bottom:20px;">
            <div class="result-card"><div class="result-label">Casos</div><div class="result-value">{stats['n']}</div><div class="result-unit">evaluados</div></div>
            <div class="result-card"><div class="result-label">MAE</div><div class="result-value">{stats['mae']:.1f}</div><div class="result-unit">dB</div></div>
            <div class="result-card"><div class="result-label">RMSE</div><div class="result-value">{stats['rmse']:.1f}</div><div class="result-unit">dB</div></div>
            <div class="result-card"><div class="result-label">Error máx.</div><div class="result-value">{stats['max_abs']:.1f}</div><div class="result-unit">dB</div></div>
            <div class="result-card"><div class="result-label">±3 dB</div><div class="result-value">{stats['within3']:.0f}%</div><div class="result-unit">casos</div></div>
            <div class="result-card"><div class="result-label">R²</div><div class="result-value">{r2_txt}</div><div class="result-unit">correlación</div></div>
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    st.markdown(
        f"""
        <div class='project-flow-card'>
            <div class='project-flow-title'>Lectura técnica</div>
            <div class='project-flow-text'>
            Error promedio firmado: <b>{stats['bias']:.1f} dB</b>. Casos dentro de ±5 dB: <b>{stats['within5']:.0f}%</b>.<br>
            La escala usada es: ±1 dB excelente, ±2 dB muy bueno, ±3 dB aceptable, sobre ±3 dB revisar/calibrar modelo.
            </div>
        </div>
        """,
        unsafe_allow_html=True
    ...
    tabs = st.tabs(["📊 Resumen estadístico", "📋 Tabla comparativa", "🔎 Caso individual", "⚙️ Biblioteca LOSCAA"])

    with tabs[0]:
        chart_df = df_res.dropna(subset=["LOSCAA", "SONARA"]).copy()
        if chart_df.empty:
            st.warning("No hay casos calculables para graficar.")
        else:
            fig = go.Figure()
            fig.add_trace(go.Scatter(
                x=chart_df["LOSCAA"],
                y=chart_df["SONARA"],
                mode="markers+text",
                text=chart_df["Código"],
                textposition="top center",
                name="Casos SONARA",
                marker=dict(size=11, color="#4F7CFF", line=dict(width=1, color="#FFFFFF")),
            ))
            min_v = float(min(chart_df["LOSCAA"].min(), chart_df["SONARA"].min())) - 3
            max_v = float(max(chart_df["LOSCAA"].max(), chart_df["SONARA"].max())) + 3
            fig.add_trace(go.Scatter(
        ...
        err_df = df_res.dropna(subset=["Error [dB]"]).copy()
        if not err_df.empty:
            fig2 = go.Figure()
            fig2.add_trace(go.Bar(
                x=err_df["Código"],
                y=err_df["Error [dB]"],
                name="Error SONARA - LOSCAA",
                marker=dict(color="#0A84FF")
            ))
            fig2.add_hline(y=0, line_dash="dash", line_color="#FFFFFF")
            fig2.add_hline(y=3, line_dash="dot", line_color="#FFD166")
            fig2.add_hline(y=-3, line_dash="dot", line_color="#FFD166")
            fig2.update_layout(
                ...
                yaxis_title="Error [dB]"
            )
            fig2.update_xaxes(tickangle=-35, gridcolor="rgba(255,255,255,.10)")
            fig2.update_yaxes(gridcolor="rgba(255,255,255,.10)")
            st.plotly_chart(fig2, use_container_width=True)

    with tabs[1]:
        show_cols = ["Código", "Solución", "Familia", "Descriptor", "LOSCAA", "SONARA", "Error [dB]", "Abs error [dB]", "Estado", "Descriptor auxiliar", "LOSCAA auxiliar", "SONARA auxiliar", "Error auxiliar [dB]"]
        st.dataframe(df_res[show_cols], use_container_width=True, hide_index=True, height=430)
        st.download_button(
    ...
    with tabs[2]:
        ...
        familia_sel = st.selectbox("Filtrar por familia", familias, key="loscaa_fam_detail")
        df_view = df.copy()
        ...
        selected = st.selectbox("Seleccionar solución LOSCAA", opciones, key="loscaa_case_detail")
        ...
        if result.get("metric") == "Ln,w":
            ...
            oficial_main = None if pd.isna(lnw_oficial) else float(lnw_oficial)
            comp_plus_label = "Ln,w+CI"
            comp_plus_sonara = result.get("lnw_ci")
            comp_plus_oficial = None if pd.isna(lnwci_oficial) else float(lnwci_oficial)
        else:
            main_label = "Rw"
            sonara_main = result.get("rw")
            oficial_main = None if pd.isna(rw_oficial) else float(rw_oficial)
            ...
            comp_plus_oficial = None if pd.isna(rw_oficial) or pd.isna(ctr_oficial) else float(rw_oficial) + float(ctr_oficial)

        delta_main = None if oficial_main is None or sonara_main is None else float(sonara_main) - float(oficial_main)
        status = loscaa_status(delta_main)
        ...
        if result.get("metric") != "Ln,w":
            ...
            rwc_oficial = None if pd.isna(rw_oficial) or pd.isna(c_oficial) else float(rw_oficial) + float(c_oficial)
    ...
    with tabs[3]:
        edited = st.data_editor(df, key="loscaa_editor", use_container_width=True, hide_index=True, num_rows="dynamic", height=420)
        ...
        with c2:
            st.download_button("⬇ Exportar biblioteca", data=df.to_csv(index=False).encode("utf-8"), file_name="loscaa_validacion.csv", mime="text/csv", use_container_width=True)
        with c3:
    ...
    st.markdown(
        """
        <div class='result-note' style='margin-top:18px;'>
        Nota: esta validación usa los valores oficiales incorporados al CSV de validación y los compara con el cálculo interno del motor SONARA. Para ampliar la validación, agrega nuevas filas con tipo_sonara implementado y valores oficiales LOSCAA.
        </div>
        """,
        unsafe_allow_html=True
    )
