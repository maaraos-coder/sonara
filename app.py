...
APP_VERSION = "1.1.2 OPTIMIZADOR + VALIDACION"
...
def sidebar(license_info=None):
    ...
    st.sidebar.markdown(
        f"<div class='sidebar-small' style='margin-top:20px;'>© 2026 SONARA * DESARROLLADO POR MAAB *<br>Versión {APP_VERSION}</div>",
        unsafe_allow_html=True
    )
...
def login_page():
    ...
    with col2:
        st.markdown("""
        <div class='card'>
            <div class='card-title'>SONARA 1.0 *DESARROLLADO POR MAAB*</div>
            <div class='card-text'>
...
def app_optimizador():
    """
    Optimizador contextual SONARA.

    Si el usuario llega desde Proyecto → Soluciones → Optimizar, el módulo queda
    amarrado al requerimiento y a la solución seleccionada. No crea un elemento nuevo
    ni cambia la matriz acústica: genera variantes para el mismo requerimiento y permite
    guardar la alternativa como nueva solución dentro de ese requerimiento.
    """
    st.markdown("<h2 style='color:#FFFFFF;margin-bottom:18px;'>Optimizador IA</h2>", unsafe_allow_html=True)

    mats = material_db()
    pid, project = active_project()
    sol_ctx = st.session_state.get("selected_solution_for_optimizer")
    req_ctx = st.session_state.get("selected_requirement_for_optimizer") or st.session_state.get("active_requirement")
    contextual = bool(sol_ctx and req_ctx)

    if contextual:
        req_id = req_ctx.get("id") or make_requirement_id(req_ctx)
        st.session_state["active_requirement_id"] = req_id
        st.session_state["active_requirement"] = req_ctx
        em = req_ctx.get("recinto_emisor", "") or "Sin emisor"
        rec = req_ctx.get("recinto_receptor", "") or "Sin receptor"
        elem = req_ctx.get("tipo_elemento", "") or req_ctx.get("elemento", "Elemento")
        indicador = str(req_ctx.get("indicador", "Rw") or "Rw")
        sentido = str(req_ctx.get("sentido", ">=") or ">=")
        try:
            objetivo_default = float(req_ctx.get("valor", sol_ctx.get("rw", sol_ctx.get("valor", 55))) or 55)
        except Exception:
            objetivo_default = float(sol_ctx.get("rw", sol_ctx.get("valor", 55)) or 55)

        actual_value = solution_descriptor_value(sol_ctx, indicador)
        estado_actual, delta_actual = solution_status(sol_ctx, req_ctx)

        st.markdown(
            f"""
            <div class='result-note'>
            <b>Optimización vinculada al proyecto:</b> {em} → {rec} · {elem}.<br>
            Requerimiento: <b>{indicador} {sentido} {objetivo_default:g} {req_ctx.get('unidad','dB')}</b> ·
            Solución base: <b>{sol_ctx.get('nombre','Solución seleccionada')}</b> ·
            Estado actual: <b>{estado_actual}</b>.
            </div>
            """,
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            "<div class='result-note'>Modo general: no hay solución/requerimiento seleccionado. Para optimizar dentro de un proyecto, entra a Proyecto → Soluciones y presiona 🧠 Optimizar sobre una solución guardada.</div>",
            unsafe_allow_html=True
        )
        indicador = "Rw"
        objetivo_default = 55.0
        actual_value = None

    # Valores iniciales desde la solución existente cuando hay contexto
    try:
        esp_actual = float(sol_ctx.get("espesor", 150) or 150) if sol_ctx else 150.0
    except Exception:
        esp_actual = 150.0
    try:
        masa_actual = float(sol_ctx.get("masa", 60) or 60) if sol_ctx else 60.0
    except Exception:
        masa_actual = 60.0
    try:
        cub = sol_ctx.get("cubicacion", {}) if isinstance(sol_ctx, dict) else {}
        area_actual = float(cub.get("area_neta", 20) or 20)
    except Exception:
        area_actual = 20.0
    ...
    with col_in:
        st.markdown("<div class='sonara-card-title'>Objetivo de optimización</div>", unsafe_allow_html=True)
        if contextual:
            st.markdown(
                "<div class='result-note'>El objetivo viene del requerimiento seleccionado. Puedes ajustar restricciones constructivas, pero la optimización queda asociada al mismo elemento del proyecto.</div>",
                unsafe_allow_html=True
            )

        objetivo_rw = st.number_input(
            "Rw objetivo equivalente [dB]",
            value=float(objetivo_default),
            min_value=25.0,
            max_value=80.0,
            step=1.0,
            help="Para DnT/DnT,A se usa como objetivo preliminar equivalente de elemento. La verificación final debe hacerse en el informe del proyecto."
        )
        max_espesor = st.number_input(
            "Espesor máximo [mm]",
            value=float(max(esp_actual * 1.25, esp_actual + 25, 80)),
            min_value=50.0,
            max_value=400.0,
            step=10.0
        )
        max_masa = st.number_input(
            "Masa máxima [kg/m²]",
            value=float(max(masa_actual * 1.35, masa_actual + 10, 30)),
            min_value=5.0,
            max_value=300.0,
            step=5.0
        )
        area = st.number_input("Superficie asociada [m²]", value=float(max(area_actual, 1)), min_value=1.0, step=1.0)
        prioridad = st.selectbox("Prioridad", ["Menor costo", "Menor espesor", "Menor peso", "Máximo aislamiento"], index=0)

        ejecutar = st.button("🧠 Buscar variantes para este requerimiento", use_container_width=True)

        if contextual:
            st.markdown("<hr>", unsafe_allow_html=True)
            st.markdown("<div class='sonara-card-title'>Solución base</div>", unsafe_allow_html=True)
            base_rw = sol_ctx.get("rw", sol_ctx.get("valor", "-"))
            st.markdown(
                f"""
                <div class='sonara-card'>
                    <b>{sol_ctx.get('nombre','Solución')}</b><br>
                    Resultado: {sol_ctx.get('resultado_label','-')}<br>
                    Masa: {sol_ctx.get('masa','-')} kg/m² · Espesor: {sol_ctx.get('espesor','-')} mm<br>
                    Valor usado como base: {base_rw} dB
                </div>
                """,
                unsafe_allow_html=True
            )
    ...
    if ejecutar:
        with st.spinner("SONARA está generando variantes para el mismo requerimiento..."):
            results = build_candidate_systems(mats, objetivo_rw, max_espesor, max_masa, area, prioridad)

        if contextual and actual_value is not None:
            try:
                actual_float = float(actual_value)
                for r in results:
                    r["Mejora vs base"] = round(float(r.get("Rw", 0)) - actual_float, 1)
            except Exception:
                for r in results:
                    r["Mejora vs base"] = None
        else:
            for r in results:
                r["Mejora vs base"] = None

        st.session_state["optimizer_last_results"] = results
        st.session_state["optimizer_last_context"] = {"contextual": contextual, "req": req_ctx, "sol": sol_ctx, "area": area, "objetivo": objetivo_rw, "prioridad": prioridad}

    results = st.session_state.get("optimizer_last_results", [])
    ...
    with col_out:
        if not results:
            st.markdown(
                """
                <div class='sonara-card'>
                    <div class='sonara-card-title'>Cómo funciona en proyecto</div>
                    <ul style='color:#DCEBFF;line-height:1.8;'>
                        <li>Parte desde una solución guardada y del requerimiento asociado.</li>
                        <li>Mantiene el mismo emisor, receptor, elemento y objetivo normativo.</li>
                        <li>Genera variantes constructivas para cumplir el requerimiento.</li>
                        <li>Ordena por costo, espesor, masa o aislamiento.</li>
                        <li>Permite guardar la variante recomendada como nueva solución del mismo requerimiento.</li>
                    </ul>
                </div>
                """,
                unsafe_allow_html=True
        ...
        st.markdown("<div class='sonara-card-title'>Variantes para el mismo requerimiento</div>", unsafe_allow_html=True)
        st.dataframe(df_res, use_container_width=True, hide_index=True, height=360)

        names = [f"{i+1}. {r['Solución']} · Rw {r['Rw']} dB · ${r['Costo estimado']:,.0f}" for i, r in enumerate(results[:15])]
        pick = st.selectbox("Seleccionar variante para guardar/cubicar", names, index=0, key="optimizer_variant_pick")
        idx = names.index(pick)
        chosen = results[idx]

        st.markdown(f"<div class='sonara-card-title'>Cubicación de variante: {chosen['Solución']}</div>", unsafe_allow_html=True)
        cub_best = chosen["cub_df"].copy()
        cub_total = float(cub_best["Subtotal"].sum()) if "Subtotal" in cub_best.columns else 0
        cub_show = pd.concat([cub_best, pd.DataFrame([{"Ítem":"TOTAL","Unidad":"","Cantidad":"","Precio unitario":"","Subtotal":round(cub_total,0)}])], ignore_index=True)
        st.dataframe(cub_show, use_container_width=True, hide_index=True, height=300)
        ...
        if contextual:
            if st.button("💾 Guardar variante optimizada en este requerimiento", use_container_width=True, key="save_optimizer_variant_to_req"):
                if not pid or not project:
                    st.error("No hay proyecto activo para guardar la variante.")
                else:
                    c = int(chosen.get("C", 0))
                    ctr = int(chosen.get("Ctr", 0))
                    rw = int(chosen.get("Rw", 0))
                    cub_payload = {
                        "codigo_partida": req_ctx.get("tipo_elemento", "Partida acústica"),
                        "tipo_cubicacion": "Optimización IA contextual",
                        "area_bruta": float(area),
                        "area_vanos": 0.0,
                        "area_neta": float(area),
                        "total": float(cub_total),
                        "fecha": datetime.now().strftime("%Y-%m-%d %H:%M"),
                        "items": cub_best.to_dict(orient="records"),
                    }
                    opt_solution = {
                        "nombre": f"Optimizada IA · {chosen['Solución']}",
                        "tipo_calculo": "Ruido aéreo",
                        "resultado_label": f"Rw {rw} dB · C {c} · Ctr {ctr} · Rw+Ctr {rw+ctr} dB",
                        "descriptor_seleccionado": "Rw",
                        "descriptor_key": "rw",
                        "valor": float(rw),
                        "rw": float(rw),
                        "c": float(c),
                        ...
                        "dntw": float(rw - 3),
                        "dnt_a": float(rw + c - 3),
                        "dnt_atr": float(rw + ctr - 3),
                        "descriptores": {"rw": float(rw), "rw_c": float(rw+c), "rw_ctr": float(rw+ctr), "stc": float(rw), "dntw": float(rw-3), "dnt_a": float(rw+c-3), "dnt_atr": float(rw+ctr-3)},
                        "masa": float(chosen.get("Masa [kg/m²]", 0)),
                        "espesor": float(chosen.get("Espesor [mm]", 0)),
                        "descripcion": f"Variante optimizada IA para requerimiento {req_ctx.get('recinto_emisor','')} → {req_ctx.get('recinto_receptor','')}. Configuración: {chosen.get('Configuración')} · Absorbente: {chosen.get('Absorbente')}",
                        "composicion": solution_composition_text({
                            "layers_left": layer_summary(chosen.get("layers_left", [])),
                            "layers_right": layer_summary(chosen.get("layers_right", [])),
                            "camara": chosen.get("camara"),
                            "configuracion": chosen.get("Configuración"),
                            "absorbente": chosen.get("Absorbente"),
                        }),
                        "layers_left": layer_summary(chosen.get("layers_left", [])),
                        "layers_right": layer_summary(chosen.get("layers_right", [])),
                        "camara": chosen.get("camara"),
                        "configuracion": chosen.get("Configuración"),
                        "absorbente": chosen.get("Absorbente"),
                        "tipo_elemento": req_ctx.get("tipo_elemento", "Paramento vertical"),
                        "origen": "Optimización IA contextual",
                        "solucion_base_id": sol_ctx.get("id"),
                        "requerimiento_id": req_ctx.get("id"),
                        "cubicacion": cub_payload,
                    }
                    st.session_state["active_requirement_id"] = req_ctx.get("id")
                    st.session_state["active_requirement"] = req_ctx
                    if save_solution_to_project(pid, opt_solution):
                        st.success("Variante optimizada guardada dentro del mismo requerimiento del proyecto.")
                    else:
                        st.error("No se pudo guardar la variante optimizada.")
        else:
            st.markdown("<div class='result-note'>Para guardar una variante dentro de un requerimiento, usa el optimizador desde Proyecto → Soluciones.</div>", unsafe_allow_html=True)
