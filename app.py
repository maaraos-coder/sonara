...
APP_VERSION = "1.1.4 OPTIMIZACION RAPIDA + VALIDACION"
...
def app_optimizador():
    ...
    selected_req = st.session_state.get("selected_requirement", None)
    selected_sol = st.session_state.get("selected_solution_for_optimization", None)

    # Fallbacks usados por versiones anteriores
    if selected_sol is None:
        selected_sol = st.session_state.get("selected_solution", None)
    if selected_req is None:
        selected_req = st.session_state.get("active_requirement", None)
    ...
    if results:
        ...
        if st.button("💾 Guardar variante optimizada en el requerimiento", use_container_width=True):
            ...
            opt_solution = {
                ...
                "descriptor": "Rw",
                "resultado": selected_variant.get("Rw", 0),
                "C": selected_variant.get("C", 0),
                "Ctr": selected_variant.get("Ctr", 0),
                "Rw+C": selected_variant.get("Rw+C", 0),
                "Rw+Ctr": selected_variant.get("Rw+Ctr", 0),
